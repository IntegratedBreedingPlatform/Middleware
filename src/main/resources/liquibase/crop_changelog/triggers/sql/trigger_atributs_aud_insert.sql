...
BEGIN
    IF (SELECT checkTableIsAudited('ATRIBUTS') = 1) THEN
        ...
        VALUES (0, NEW.aid, NEW.gid, NEW.atype, NEW.created_by, NEW.aval, NEW.cval_id, NEW.alocn, NEW.aref, NEW.adate, NEW.created_date, NEW.modified_date, NEW.modified_by);
    END IF;
