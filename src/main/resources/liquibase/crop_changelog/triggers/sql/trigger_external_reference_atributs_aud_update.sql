...
CREATE TRIGGER trigger_external_reference_atributs_aud_update
    AFTER UPDATE
...
BEGIN
    IF (SELECT checkTableIsAudited('EXTERNAL_REFERENCE_ATRIBUTS') = 1) THEN
        ...
        VALUES (1, NEW.id, NEW.aid, NEW.reference_id, NEW.reference_source, NEW.created_by, NEW.modified_by, NEW.created_date, NEW.modified_date);
    END IF;
