...
BEGIN
    IF (SELECT checkTableIsAudited('PHENOTYPE') = 1) THEN
        ...
        VALUES (1, NEW.phenotype_id, NEW.uniquename, NEW.name, NEW.observable_id, NEW.attr_id, NEW.value, NEW.cvalue_id, NEW.status, NEW.assay_id, NEW.nd_experiment_id, NEW.created_date, NEW.updated_date, NEW.draft_value, NEW.draft_cvalue_id, NEW.created_by, NEW.updated_by, NEW.json_props);
    END IF;
