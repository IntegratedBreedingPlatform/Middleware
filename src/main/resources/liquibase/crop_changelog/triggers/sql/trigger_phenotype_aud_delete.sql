...
BEGIN
    IF (SELECT checkTableIsAudited('PHENOTYPE') = 1) THEN
        ...
        VALUES (2, OLD.phenotype_id, OLD.uniquename, OLD.name, OLD.observable_id, OLD.attr_id, OLD.value, OLD.cvalue_id, OLD.status, OLD.assay_id, OLD.nd_experiment_id, OLD.created_date, @updated_date, OLD.draft_value, OLD.draft_cvalue_id, OLD.created_by, @updatedBy);
    END IF;
