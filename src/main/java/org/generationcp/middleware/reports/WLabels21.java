	...
	protected String buildRecord(List<List<String>> rows, List<String> headers, int colSpan){
		...
		for(int i = 0; i<columns; i++){
			sb.append(StringUtil.format(records.get(i).get("subProg"), 3, true)).append(" ")
			  ...
			  .append(StringUtil.format(records.get(i).get("season"), 6, true))
			  .append(StringUtil.format("PLOT", 5, false)).append(" ")
			  .append(StringUtil.format(records.get(i).get("plot"), 6, true))
