...
public class GenotypeDao extends GenericDAO<Genotype, Integer> {
    ...
    public List<GenotypeDTO> searchGenotypes(final GenotypeSearchRequestDTO searchRequestDTO, final Pageable pageable) {
        ...
        if(!CollectionUtils.isEmpty(searchRequestDTO.getFilter().getVariableMap())) {
            for(final String varName: searchRequestDTO.getFilter().getVariableMap().values()) {
                final StringBuilder genotypeVariablesClause = new StringBuilder(", MAX(IF(cvterm_variable.name = '%1$s', geno.value, NULL)) AS `%1$s`,")
    ...
    private List<GenotypeDTO> getGenotypeResults(final GenotypeSearchRequestDTO searchRequestDTO, List<Map<String, Object>> results) {
