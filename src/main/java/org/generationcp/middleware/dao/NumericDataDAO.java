...
public class NumericDataDAO extends GenericDAO<NumericData, NumericDataPK> {
    ...
    public List<NumericDataElement> getValuesByOunitIDList(
	    List<Integer> ounitIdList) throws QueryException {
	try {
	    SQLQuery query = getSession().createSQLQuery(
		    NumericData.GET_BY_OUNIT_ID_LIST);
		...
		if (result != null) {
		    ...
		    NumericDataElement dataElement = new NumericDataElement(
	    ...
	    throw new QueryException(
		    "Error with get Numeric Data Values by list of Observation Unit IDs query: "
			    + ex.getMessage());
	}
    }
}
