...
public class GermplasmDAO extends GenericDAO<Germplasm, Integer> 
{
	...
	public List<Germplasm> findByName(String name, int start, int numOfRows, Operation operation, Integer status, GermplasmNameType type)
	{
		...
		if(operation == null || operation == Operation.EQUAL)
			nameCriteria.add(Restrictions.eq("nval", name));
		else if(operation == Operation.LIKE)
			nameCriteria.add(Restrictions.like("nval", name));
		...
		if(type != null)
		{
			Criteria typeCriteria = nameCriteria.createCriteria("type");
			Integer typeid = type.getUserDefinedFieldID();
	...
	/**
	@SuppressWarnings("unchecked")
	public List<Germplasm> findByExample(Germplasm sample, int start, int numOfRows)
	{
		Example sampleGermplasm = Example.create(sample)
			.ignoreCase()
			.enableLike(MatchMode.ANYWHERE);
		
		Criteria mainCriteria = getSession().createCriteria(Germplasm.class);
		mainCriteria.add(sampleGermplasm);
		
		if(sample.getMethod() != null)
		{
			Example sampleMethod = Example.create(sample.getMethod())
				.ignoreCase()
				.enableLike(MatchMode.ANYWHERE);
			
			mainCriteria.createCriteria("method").add(sampleMethod);
		}
		
		if(sample.getLocation() != null)
		{
			Example sampleLocation = Example.create(sample.getLocation())
			.ignoreCase()
			.enableLike(MatchMode.ANYWHERE);
		
			mainCriteria.createCriteria("location").add(sampleLocation);
		}
		
		if(sample.getUser() != null)
		{
			Example sampleUser = Example.create(sample.getUser())
			.ignoreCase()
			.enableLike(MatchMode.ANYWHERE);
		
			mainCriteria.createCriteria("user").add(sampleUser);
		}
		
		if(sample.getReference() != null)
		{
			Example sampleRef = Example.create(sample.getReference())
			.ignoreCase()
			.enableLike(MatchMode.ANYWHERE);
		
			mainCriteria.createCriteria("reference").add(sampleRef);
		}
		
		if(sample.getAttributes() != null && !sample.getAttributes().isEmpty())
		{
			Set<Attribute> attrs = sample.getAttributes();
			Criteria attributesCriteria = mainCriteria.createCriteria("attributes");
			for(Attribute attr : attrs)
			{
				Example sampleAttribute = Example.create(attr)
				.ignoreCase()
				.enableLike(MatchMode.ANYWHERE);
	...
	public Long countByExample(Germplasm sample)
	{
		Example sampleGermplasm = Example.create(sample)
			.ignoreCase()
			.enableLike(MatchMode.ANYWHERE);
		...
		Long count = (Long) mainCriteria.uniqueResult();
		return count;
	}
	**/
}
