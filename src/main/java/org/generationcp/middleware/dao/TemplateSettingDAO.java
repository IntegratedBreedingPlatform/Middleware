...
public class TemplateSettingDAO extends GenericDAO<TemplateSetting, Integer> {
	...
	public TemplateSetting getByName(String name) throws MiddlewareQueryException {
		try {
			Criteria criteria = this.getSession().createCriteria(TemplateSetting.class).add(Restrictions.eq("name", name)).setMaxResults(1);
			return (TemplateSetting) criteria.uniqueResult();
		} catch (HibernateException e) {
			this.logAndThrowException("Error with getByName(name=" + name + ") query from TemplateSetting: " + e.getMessage(), e);
		}
		return null;
	}
	...
	public List<TemplateSetting> getByTool(Tool tool) throws MiddlewareQueryException {
		try {
			Criteria criteria = this.getSession().createCriteria(TemplateSetting.class).add(Restrictions.eq("tool", tool));
			return criteria.list();
		} catch (HibernateException e) {
			this.logAndThrowException("Error with getByTool(tool=" + tool + ") query from TemplateSetting: " + e.getMessage(), e);
		}
		return null;
	}
	...
	public List<TemplateSetting> getByProjectId(Integer projectId) throws MiddlewareQueryException {
		try {
			Criteria criteria = this.getSession().createCriteria(TemplateSetting.class).add(Restrictions.eq("projectId", projectId));
			return criteria.list();
		} catch (HibernateException e) {
			this.logAndThrowException(
					"Error with getByProjectId(projectId=" + projectId + ") query from TemplateSetting: " + e.getMessage(), e);
		}
		return null;
	}
	...
	public List<TemplateSetting> get(TemplateSetting filter) throws MiddlewareQueryException {
		...
		try {
			Criteria criteria = this.getSession().createCriteria(TemplateSetting.class);
			...
			this.logAndThrowException("Error with get(templateFilter=" + filter + ") query from TemplateSetting: " + e.getMessage(), e);
		}
