...
public class CharacterDataDAO extends
	GenericDAO<CharacterData, CharacterDataPK> {
	...
	for (TraitCombinationFilter combination : filters) {
		...
		crit.add(Restrictions.eq("variate.traitId",
			combination.getTraitId()));
		crit.add(Restrictions.eq("variate.scaleId",
			combination.getScaleId()));
		crit.add(Restrictions.eq("variate.methodId",
			combination.getMethodId()));
    ...
    public List<CharacterDataElement> getValuesByOunitIDList(
	    List<Integer> ounitIdList) throws QueryException {
	try {
	    SQLQuery query = getSession().createSQLQuery(
		    CharacterData.GET_BY_OUNIT_ID_LIST);
		...
		if (result != null) {
		    ...
		    CharacterDataElement dataElement = new CharacterDataElement(
			    ounitId, variateId, variateName, value);
	    ...
	    throw new QueryException(
		    "Error with get Character Data Values by list of Observation Unit IDs query: "
			    + ex.getMessage());
	}
    }
}
