...
public class ExtendedMarkerInfoDAO extends GenericDAO<ExtendedMarkerInfo, Integer> {
    public static final String SELECT_STRING = "SELECT marker_id " +
            ", CONCAT(marker_type, '') " +
            ", CONCAT(marker_name, '')  " +
            ", CONCAT(species, '')  " +
            ", db_accession_id " +
    ...
    public static final String GET_BY_MARKER_TYPE = SELECT_STRING + FROM_STRING
            + " where marker_type = :markerType";

    public static final String COUNT_BY_MARKER_TYPE = "SELECT COUNT(*) " + FROM_STRING + " where marker_type = :markerType";

    public static final String GET_LIKE_MARKER_NAME = SELECT_STRING + FROM_STRING + " where LOWER(marker_name) LIKE LOWER(:markerName)";

    public static final String GET_BY_MARKER_NAMES = SELECT_STRING + FROM_STRING + " where marker_name in (:markerNames)";
    ...
    public List<ExtendedMarkerInfo> getByMarkerType(String type/*, int start, int numOfRows*/) throws MiddlewareQueryException {
        ...
        try {
            ...
            logAndThrowException("Error with getMarkerInfoByMarkerType(markerType=" + type
                    + ") query from ExtendedMarkerInfo: " + e.getMessage(), e);
        }
    ...
    public List<ExtendedMarkerInfo> getLikeMarkerName(String partialMarkerName) throws MiddlewareQueryException {
        ...
        try {
            SQLQuery query = getSession().createSQLQuery(GET_LIKE_MARKER_NAME);
            ...
            logAndThrowException("Error with getLikeMarkerName(markerName=" + partialMarkerName
    ...
    public List<ExtendedMarkerInfo> getByMarkerNames(List<String> markerNames) throws MiddlewareQueryException {
        ...
        try {
            SQLQuery query = getSession().createSQLQuery(GET_BY_MARKER_NAMES);
            ...
            logAndThrowException("Error with getByMarkerNames(markerNames=" + markerNames
                    + ") query from MarkerInfo: " + e.getMessage(), e);
        }
        return new ArrayList<ExtendedMarkerInfo>();
    }
    ...
    public long countByMarkerType(String markerType) throws MiddlewareQueryException {
        ...
        try {
            SQLQuery query = getSession().createSQLQuery(COUNT_BY_MARKER_TYPE);
            ...
            logAndThrowException("Error with countByMarkerType(markerType=" + markerType + ") query from MarkerInfo: " + e.getMessage(), e);
        }
    ...
    protected ExtendedMarkerInfo convertFromObject(Object[] result) {
        ...
        ExtendedMarkerInfo markerInfo = new ExtendedMarkerInfo(markerId, markerType, markerName2, species, accessionId, reference, genotype,
                ...
                numberOfRepeats, motifType, sequence, sequenceLength, minAllele, maxAllele, ssrNumber, forwardPrimerTemp, reversePrimerTemp, elongationTemp, fragmentSizeExpected,
                fragmentSizeObserved, expectedProductSize, positionOnSequence, restrictionEnzyme);
