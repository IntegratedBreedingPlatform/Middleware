...
public class LotDAO extends GenericDAO<Lot, Integer> {
	...
	private final String SEARCH_LOT_QUERY = "SELECT lot.lotid as lotId, " //
		...
		+ "  SUM(CASE WHEN transaction.trnstat = " + TransactionStatus.PENDING.getIntValue() +" AND transaction.trnqty > 0 THEN transaction.trnqty ELSE 0 END) AS actualBalance, " //
		...
		+ "  MAX(CASE WHEN transaction.trnstat = " + TransactionStatus.PENDING.getIntValue() + " AND transaction.trnqty > 0 THEN transaction.trndate ELSE null END) AS lastDepositDate, " //
	...
	private String buildSearchLotsQuery(final LotsSearchDto lotsSearchDto) {
		...
		if (lotsSearchDto != null) {
			...
			if (lotsSearchDto.getMinActualBalance() != null) {
				query.append("and SUM(CASE WHEN transaction.trnstat = " + TransactionStatus.PENDING.getIntValue() +" AND transaction.trnqty > 0 THEN transaction.trnqty ELSE 0 END) >= ")
			...
			if (lotsSearchDto.getLastDepositDateFrom() != null) {
				query.append(
						" and DATE(MAX(CASE WHEN transaction.trnstat = " + TransactionStatus.PENDING.getIntValue() +" AND transaction.trnqty > 0 THEN transaction.trndate ELSE null END)) >= '")
						.
