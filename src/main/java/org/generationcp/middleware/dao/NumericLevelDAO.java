...
public class NumericLevelDAO extends GenericDAO<NumericLevel, NumericLevelPK> {
    ...
    public List<NumericLevelElement> getValuesByOunitIDList(
	    List<Integer> ounitIdList) throws QueryException {
	try {
	    SQLQuery query = getSession().createSQLQuery(
		    NumericLevel.GET_BY_OUNIT_ID_LIST);
		...
		if (result != null) {
		    ...
		    NumericLevelElement levelElement = new NumericLevelElement(
	    ...
	    throw new QueryException(
		    "Error with get Numeric Level Values by list of Observation Unit IDs query: "
			    + ex.getMessage());
	}
    }
}
