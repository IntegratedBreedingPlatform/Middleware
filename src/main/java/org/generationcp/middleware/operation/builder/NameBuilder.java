...
public class NameBuilder extends Builder {
	...
	public int getMaximumSequence(boolean isBulk, String prefix, String suffix, int count) throws MiddlewareQueryException {
		...
		names.addAll(this.getNameDao().getAllMatchingNames(prefix, suffix));
		timer.restart("get maximum sequence");
		int max = this.getMaximumSequenceInNames(isBulk, names, prefix, suffix, count);
		timer.stop();
		return max;
	}
	...
	private int getMaximumSequenceInNames(boolean isBulk, Set<String> names, String prefix, String suffix, int plantsSelected) {
		...
		if (isBulk) {
			regex1 =
					"^\\Q" + prefix + "\\E" + (plantsSelected > 1 ? "[" + plantsSelected + "]" : "") + "\\Q" + suffix + "\\E"
							+ "[\\(]?([\\d\\s]*)[\\)]?$";
			regex2 =
					"^\\Q" + standardizedPrefix + "\\E" + (plantsSelected > 1 ? "[" + plantsSelected + "]" : "") + "\\Q"
							+ standardizedSuffix + "\\E" + "[\\(]?([\\d\\s]*)[\\)]?$";
			regex3 =
					"^\\Q" + prefix.replaceAll("\\s", "") + "\\E" + (plantsSelected > 1 ? "[" + plantsSelected + "]" : "") + "\\Q"
							+ suffix.replaceAll("\\s", "") + "\\E" + "[\\(]?([\\d\\s]*)[\\)]?$";
		} else {
