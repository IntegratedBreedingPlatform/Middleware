					...
					if (tInfo.getFieldMapLabels() != null) {
						for (FieldMapLabel label : tInfo.getFieldMapLabels()) {
							...
							if (label.getColumn() != null && label.getRange() != null) {
								ExperimentModel experiment = this.getExperimentBuilder().getExperimentModel(label.getExperimentId());
								this.saveOrUpdateProperty(experiment, TermId.COLUMN_NO, String.valueOf(label.getColumn()));
								this.saveOrUpdateProperty(experiment, TermId.RANGE_NO, String.valueOf(label.getRange()));
							}
						}
					}
				}
