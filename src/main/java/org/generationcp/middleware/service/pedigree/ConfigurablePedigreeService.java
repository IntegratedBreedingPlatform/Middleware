...
import org.generationcp.middleware.pojos.germplasm.BackCrossOrNormalCross;
import org.generationcp.middleware.pojos.germplasm.NewBackcrossElement;
import org.generationcp.middleware.pojos.germplasm.NewGermplasmCross;
import org.generationcp.middleware.pojos.germplasm.NewGermplasmCrossElement;
import org.generationcp.middleware.pojos.germplasm.NewSingleGermplasmCrossElement;
...
public class ConfigurablePedigreeService implements PedigreeService {
	...
	public String getCrossExpansion(Integer gid, Integer level, CrossExpansionProperties crossExpansionProperties) {
		...
		if (germplasm != null) {
			...
			final NewSingleGermplasmCrossElement startElement = new NewSingleGermplasmCrossElement();
			//startElement.setGermplasm(germplasm);
			setMetaData(startElement, germplasm, true);

			NewGermplasmCrossElement cross =
	...
	private NewGermplasmCrossElement expandGermplasmCross(NewGermplasmCrossElement element, int rootLevel, int currentLevel, boolean forComplexCross) {
		if (currentLevel == 0) {
			...
			if (element instanceof NewSingleGermplasmCrossElement) {
				// No meta data attached to this single class as it is passed in
				NewSingleGermplasmCrossElement singleGermplasm = (NewSingleGermplasmCrossElement) element;
				...
				if (germplasmToExpand.getGnpgs() < 0) {
					...
					if (germplasmToExpand.getGpid1() != null && germplasmToExpand.getGpid1() != 0 && !forComplexCross) {
						NewSingleGermplasmCrossElement nextElement = new NewSingleGermplasmCrossElement();
				...
				// If this is a generative crossing
					...
					final NewGermplasmCross cross = new NewGermplasmCross();
					...
					if (method != null) {
						...
						if (methodName.contains("single cross")) {
							...
							NewBackcrossElement generateBackcross = generateBackcross(element, rootLevel, currentLevel, forComplexCross, germplasmToExpand, cross);
							...
							NewSingleGermplasmCrossElement crossElement = new NewSingleGermplasmCrossElement();
	...
	private void processNormalCross(NewGermplasmCrossElement element, int level, Germplasm germplasmToExpand, final NewGermplasmCross cross) {
		...
		NewSingleGermplasmCrossElement firstParentElem = new NewSingleGermplasmCrossElement();
		//firstParentElem.setGermplasm(firstParent);
		setMetaData(firstParentElem, firstParent, false);

		NewSingleGermplasmCrossElement secondParentElem = new NewSingleGermplasmCrossElement();
	...
	private void processComplexCross(NewGermplasmCrossElement element, int rootLevel, int level, Germplasm germplasmToExpand, final NewGermplasmCross cross) {
		...
		// level
		NewGermplasmCrossElement expandedFirstParent = this.expandGermplasmCross(firstParentElem, rootLevel, level, true);
		NewGermplasmCrossElement expandedSecondParent = this.expandGermplasmCross(secondParentElem, rootLevel, level, true);

		// get the number of crosses in the first parent
		int numOfCrosses = 0;
		if (expandedFirstParent instanceof NewGermplasmCross) {
			numOfCrosses = ((NewGermplasmCross) expandedFirstParent).getNumberOfCrossesBefore() + 1;
		}
	...
	private NewBackcrossElement generateBackcross(NewGermplasmCrossElement element, int rootLevel, int currentLevel, boolean forComplexCross, Germplasm germplasmToExpand,
			final NewGermplasmCross cross) {
		NewBackcrossElement backcross = new NewBackcrossElement();
		...
		if (firstParent != null && secondParent != null) {
			NewSingleGermplasmCrossElement recurringParentElem = new NewSingleGermplasmCrossElement();
			NewSingleGermplasmCrossElement parentElem = new NewSingleGermplasmCrossElement();
			...
			if (itsABackCross) {
				NewGermplasmCrossElement expandedRecurringParent =
				...
				NewGermplasmCrossElement expandedParent =
	...
	private void processThreeWayCross(NewGermplasmCrossElement element, int rootLevel, int level, boolean forComplexCross, Germplasm germplasmToExpand,
			final NewGermplasmCross cross) {
		...
		if (firstParent != null && firstParent.getGnpgs() > 0) {
			...
			Germplasm firstGrandParent =
					this.pedigreeDataManagerFactory.getGermplasmDataManager().getGermplasmWithPrefName(
							firstParent.getGpid1());
			NewSingleGermplasmCrossElement firstGrandParentElem = new NewSingleGermplasmCrossElement();
			...
			Germplasm secondGrandParent =
					this.pedigreeDataManagerFactory.getGermplasmDataManager().getGermplasmWithPrefName(
							firstParent.getGpid2());
			NewSingleGermplasmCrossElement secondGrandParentElem = new NewSingleGermplasmCrossElement();
			...
			// on the level
			NewGermplasmCrossElement expandedFirstGrandParent =
					this.expandGermplasmCross(firstGrandParentElem, rootLevel, level - 1, forComplexCross);
			NewGermplasmCrossElement expandedSecondGrandParent =
			...
			NewGermplasmCross crossForGrandParents = new NewGermplasmCross();
			...
			if (expandedFirstGrandParent instanceof NewGermplasmCross) {
				numOfCrossesForGrandParents =
						((NewGermplasmCross) expandedFirstGrandParent).getNumberOfCrossesBefore() + 1;
			}
	...
	private void processDoubleCross(NewGermplasmCrossElement element, int rootLevel, int level, boolean forComplexCross, Germplasm germplasmToExpand,
			final NewGermplasmCross cross) {
		...
		NewSingleGermplasmCrossElement thirdGrandParentElem = new NewSingleGermplasmCrossElement();
		NewSingleGermplasmCrossElement fourthGrandParentElem = new NewSingleGermplasmCrossElement();
		...
		// the level
		NewGermplasmCrossElement expandedFirstGrandParent = null;
		NewGermplasmCrossElement expandedSecondGrandParent = null;
		NewGermplasmCrossElement expandedThirdGrandParent = null;
		NewGermplasmCrossElement expandedFourthGrandParent = null;
		...
		// grand parents
		NewGermplasmCross firstCross = new NewGermplasmCross();
		...
		if (firstParent != null) {
			...
			// cross
			if (expandedFirstGrandParent instanceof NewGermplasmCross) {
				numOfCrossesForFirst = ((NewGermplasmCross) expandedFirstGrandParent).getNumberOfCrossesBefore() + 1;
			}
		...
		// grand parents
		NewGermplasmCross secondCross = new NewGermplasmCross();
		if (secondParent != null) {
			...
			if (expandedThirdGrandParent instanceof NewGermplasmCross) {
				numOfCrossesForSecond = ((NewGermplasmCross) expandedThirdGrandParent).getNumberOfCrossesBefore() + 1;
			}
		...
		if (firstParent != null) {
			...
			if (expandedSecondGrandParent instanceof NewGermplasmCross) {
				numOfCrosses =
						numOfCrosses + ((NewGermplasmCross) expandedSecondGrandParent).getNumberOfCrossesBefore() + 1;
			}
		}
		cross.setNumberOfCrossesBefore(numOfCrosses);
	}

	private void processParent(NewGermplasmCrossElement element, int level, Germplasm parent,
			NewSingleGermplasmCrossElement firstGrandParentElem, NewSingleGermplasmCrossElement secondGrandParentElem) {
		Germplasm firstGrandParent;
		Germplasm secondGrandParent;
		if (parent != null) {
			firstGrandParent =
	...
	private void processSingleCross(NewGermplasmCrossElement element, int rootLevel, int level, boolean forComplexCross, Germplasm germplasmToExpand,
			final NewGermplasmCross cross) {
		// get the immediate parents
		Germplasm firstParent =
				this.pedigreeDataManagerFactory.getGermplasmDataManager().getGermplasmWithPrefName(
...
//		secondParentElem.setGermplasm(secondParent);
		...
		// expand the parents as needed, depends on the
		// level
		NewGermplasmCrossElement expandedFirstParent =
				this.expandGermplasmCross(firstParentElem, rootLevel, level - 1, forComplexCross);
		NewGermplasmCrossElement expandedSecondParent =
	...
	private void setMetaData(BackCrossOrNormalCross childElement, Germplasm germplasm, boolean rootNode) {
