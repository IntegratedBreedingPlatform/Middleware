...
public class PedigreeServiceImpl extends Service implements PedigreeService {
	...
	@Override
	public String getCrossExpansion(Integer gid, CrossExpansionRule crossExpansionRule)
		...
		if (germplasm != null) {
			if (crossExpansionRule.isCimmytWheat()) {
				...
				GermplasmCrossElement cross = expandGermplasmCross(startElement,
						crossExpansionRule.getStopLevel(), false);
				return cross.toString();
			}
		} else {
			return null;
	...
	private List<Name> getCimmytWheatWayNamesList(int gid, List<Integer> ntypeArray,
			List<Integer> nstatArray, List<Integer> nuiArray) throws MiddlewareQueryException {

		List<Name> nameList = getNameDao().getByGIDWithFilters(gid, null, null);
		List<Name> returnNameList = new ArrayList<Name>();
		for (Name name : nameList) {
			if (name.getNstat() != 9) {
				if (ntypeArray.contains(name.getTypeId()) && nstatArray.contains(name.getNstat())
	 ...
	 *            Input GID
	 * @param nivel
	 *            default zero is level we are at currently
	 * @param gpidinfClass
	 *            empty class
	 * @param fback
	 *            default zero
	 ...
	 * @throws Exception
	 */
	public String arma_pedigree(int p_gid, int nivel, Germplasm gpidinfClass, int fback, int mback,
		...
		// listNL = getNamesByGID(grTemp.getGid(), null,
		// getGermplasmType(ntype));
		// Determine if there was a Female or Male Backcross, that should be
		// used in the pedigree, then the proper name of the
		// line can't be used. For instance if the name is 27217-A-4-8 and the
		// pedigree is also MT/BRT and we are
		// called with mback representing BRT, then we must retrieve MT/BRT
		// instead of 27217-A-4-8 to be able to reproduce
		// the name MT/2*BRT as the final pedigree. See example with GID=1935,
		// before it generated 27217-A-4-8/BRT, now it
		// generates MT/2*BRT as expected.

		
		// GID has a backcross 
		if (grTemp.getGnpgs() == 2 && (fback != 0 || mback !=0)) {

			if ((grTemp.getGpid1() == fback) && (fback != 0)) {
				BackCrossFound = true;
			}
			if ((grTemp.getGpid2() == mback) && (mback != 0)) {
				BackCrossFound = true;
			}
			// ' Also handle the CIMMYT retrocrosses A/B//A is A*2/B and B//A/B
			// is
			// A/2*B JEN 2012-02-17
			if (grTemp.getGpid2() == fback && fback != 0) {
				BackCrossFound = true;
			}
			if (grTemp.getGpid1() == mback && mback != 0) {
				BackCrossFound = true;
			}
		}
		// Pristeen name list
		listNL = getCimmytWheatWayNamesList(grTemp.getGid(),
				Arrays.asList(cimmytWheatNameUtil.getNtypeArray()),
				Arrays.asList(cimmytWheatNameUtil.getNstatArray()),
				Arrays.asList(cimmytWheatNameUtil.getNuidArray()));

		// Deravite and maintence process
		if ((!listNL.isEmpty()) && (grTemp.getGnpgs() == -1)) {
			...
			if (grTemp.getGpid1() != null && grTemp.getGpid1() != 0) {
				...
				if (rs_back.getGid() == null) {
					...
					// New artificial backcrosses implemented 2 If's JEN -
					// 2012-02-13
		...
		// If the current GID is identical to one of the backcross GIDs that
		// must be respected,
		...
		// If the backcross is not found get the preferred name get the best name form the weight calculation
		if ((!listNL.isEmpty()) && !(BackCrossFound)) {
			...
			for (Name namesrecord : listNL) {
				...
				// If that is the case and we are at level zero, add 200 to
				// xCurrent if NSTAT=1
				if ((nivel == 0) && cimmytWheatNameUtil.isLevelZeroFullName()
						&& (namesrecord.getNstat() == 1)) {
			...
			if ((grTemp.getGpid1() == 0) && (grTemp.getGpid2() == 0)) {
				...
				// expand the male if not found then female 
				if ((grTemp.getGnpgs() == -1) && (grTemp.getGpid2() != 0)) {
					ped = arma_pedigree(grTemp.getGpid2(), nivel, gpidinfClass, fback, mback,
							Resp1, Resp2, ntype, cimmytWheatNameUtil);
				} else if ((grTemp.getGnpgs() == -1) && (grTemp.getGpid1() != 0)) {
					ped = arma_pedigree(grTemp.getGpid1(), nivel, gpidinfClass, fback, mback,
							Resp1, Resp2, ntype, cimmytWheatNameUtil);
				} else {
					gpidinfClass.setGpid1(grTemp.getGpid1());
					gpidinfClass.setGpid2(grTemp.getGpid2());
					if (grTemp.getGpid1() == fback) {
						p1 = arma_pedigree(grTemp.getGpid1(), nivel + 1, fGpidInfClass, 0, 0,
								Resp1, Resp2, ntype, cimmytWheatNameUtil);
					} else {
						p1 = arma_pedigree(grTemp.getGpid1(), nivel + 1, fGpidInfClass, 0,
								grTemp.getGpid2(), Resp1, Resp2, ntype, cimmytWheatNameUtil);
					}
					if (grTemp.getGpid2() == mback) {
						p2 = arma_pedigree(grTemp.getGpid2(), nivel + 1, mGpidInfClass, 0, 0,
								Resp1, Resp2, ntype, cimmytWheatNameUtil);
					} else {
						p2 = arma_pedigree(grTemp.getGpid2(), nivel + 1, mGpidInfClass,
								grTemp.getGpid1(), 0, Resp1, Resp2, ntype, cimmytWheatNameUtil);
					}
				}
				// ' Since female/male backcross is a bit meaningless when IWIS2
				// false backrosses are handled, then
				// ' we just detect which part could be handled by length of p1
				// and p2, and if they are contained in
				...
				if (grTemp.getGpid1().intValue() == mGpidInfClass.getGpid1().intValue()
						...
						|| grTemp.getGpid1().intValue() == mGpidInfClass.getGpid2().intValue())// Handle
																								// Backcross
				{
					veces_rep = 2;
					if ((!p1.equals("")) && (!p2.equals(""))) {
						if ((!p1.contains(p2)) && (!p2.contains(p1))) {
							...
							mGpidInfClass.setGpid2(0);
							p1 = arma_pedigree(grTemp.getGpid1(), nivel + 1, fGpidInfClass, 0, 0,
									grTemp.getGpid1(), grTemp.getGpid2(), ntype,
									cimmytWheatNameUtil);
							p2 = arma_pedigree(grTemp.getGpid2(), nivel + 1, mGpidInfClass, 0, 0,
									grTemp.getGpid1(), grTemp.getGpid2(), ntype,
									cimmytWheatNameUtil);
							if ((!p1.contains(p2)) && (!p2.contains(p1))) {
								...
								// Resolving situation of GID=29367 CID=22793
								// and GID=29456 CID=22881
								// Female : CMH75A.66/2*CNO79 Male
								// CMH75A.66/3*CNO79 Result:
								// CMH75A.66/2*CNO79*2//CNO79
								// Solution: convert p1 and p2 to the following
								// p1 : CMH75A.66/2*CNO79 p2:
								// CMH75A.66/2*CNO79//CNO79
								//
								// A valid way to pass parameters by reference
								// according to
						...
						if (LongStr.substring(LongStr.length() - ShortStr.length())
								.equals(ShortStr)) {
							...
							if (cut.endsWith("/")) {
								...
								if (cut.substring(cut.length() - 3, cut.length() - 2).equals("/")) {
									veces_rep = Integer.valueOf(cut.substring(cut.length() - 2,
											cut.length() - 1));
									cut = cut.substring(0, (cut.length() - 2));
								} else {
									veces_rep = Integer.valueOf(cut.substring(cut.length() - 3,
											cut.length() - 1));
				...
				if ((grTemp.getGpid1().intValue() != mGpidInfClass.getGpid1().intValue())
						...
						&& (grTemp.getGpid2().intValue() != fGpidInfClass.getGpid2().intValue())
						&& (grTemp.getGpid2().intValue() != fGpidInfClass.getGpid2().intValue())
						&& (grTemp.getGpid2().intValue() != mGpidInfClass.getGpid2().intValue())) {
	...
	private void GetParentsDoubleRetroCrossNew(String[] Mxp1, String[] Mxp2) { // 24
																				// agosto
																				// 2012,
																				// Medificaciones
																				// por
		...
		// Female : CMH75A.66/2*CNO79 Male CMH75A.66/3*CNO79 Result:
		// CMH75A.66/2*CNO79*2//CNO79
		// Solution:
		...
		boolean SlashLeft = true; // /2*A is SlashLeft, and A*2/ is not
									// SlashLeft
		...
		for (x = 1; x <= xp1.length(); x++) {
			if (xp1.substring(x - 1, x).equals(xp2.substring(x - 1, x))) {
				...
				if (BeforeStr.length() > 1) {
					if (NumberUtils.isNumber(BeforeStr.substring(BeforeStr.length() - 1,
							BeforeStr.length()))
							&& BeforeStr.endsWith("*")) {
						Lev1 = Integer.valueOf(BeforeStr.substring(BeforeStr.length() - 1,
								BeforeStr.length()));
						xDel = "*";
					}
				}
				...
				if (xDel.equals("*") || xDel.equals("/")) {
					for (y = xx; y < xp1.length(); y++) {
						...
						// Take off leading offset in AfterStr if that has now
						// been put in Lev2
			...
			if (xp1.substring(x - 1, x).equals(CutStr.substring(x - 1, x))) {
				...
				if (BeforeStr.length() > 1) {
					...
					if (NumberUtils.isNumber(BeforeStr.substring(BeforeStr.length() - 1,
							BeforeStr.length()))
							&& BeforeStr.endsWith("*")) {
						Lev2 = Integer.valueOf(BeforeStr.substring(BeforeStr.length() - 1,
								BeforeStr.length()));
						xDel = "*";
					}
				}
				...
				if (Lev2 == 0) {
					...
					while ((AfterStr.length() > 2) && Changed) {
						...
						if ((A.equals("/") || (NumberUtils.isNumber(A)))
								&& (B.equals("/") || (NumberUtils.isNumber(B)))) {
							AfterStr = AfterStr.substring(1, AfterStr.length());
							Changed = true;
						}
					}
		...
		// Fixing CleanAfter
		// if (CleanAfter.substring(0,1).equals("*") ||
		// CleanAfter.substring(0,1).equals("/")) {
		...
		if (CleanBefore.length() > 1) {
			...
			if (NumberUtils.isNumber(CleanBefore.substring(CleanBefore.length() - 1,
					CleanBefore.length()))
					&& ((CleanBefore.substring(CleanBefore.length() - 2, CleanBefore.length() - 1)
							.equals("/")) || (CleanBefore.substring(CleanBefore.length() - 1,
							CleanBefore.length()).equals("*")))) {
	...
	@Override
	public String getCrossExpansionCimmytWheat(int gid, int level, int type)
			throws MiddlewareQueryException {
		try {
