...
public class PhenotypeQuery {
	...
	public static final String PHENOTYPE_SEARCH = " SELECT " //
		...
		+ "  (SELECT l.locid FROM nd_geolocationprop gp INNER JOIN location l ON l.locid = gp.value WHERE gp.type_id = 8190 AND gp.nd_geolocation_id = gl.nd_geolocation_id) AS studyLocationDbId, " //
		+ "  (SELECT l.lname FROM nd_geolocationprop gp INNER JOIN location l ON l.locid = gp.value WHERE gp.type_id = 8190 AND gp.nd_geolocation_id = gl.nd_geolocation_id) AS studyLocation, " //
	...
	public static final String PHENOTYPE_SEARCH_COUNT = " " //
		;
