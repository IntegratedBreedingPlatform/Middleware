...
/**
 * Implementation of the GermplasmDataManager interface.  To instantiate this class, a Hibernate
 * Session must be passed to its constructor.
 * 
...
public class GermplasmDataManagerImpl implements GermplasmDataManager
{
	...
	private void updateGermplasmPrefNameAbbrev(Integer gid, String newPrefValue, String nameOrAbbrev) throws QueryException
	{
		...
		try {
			...
			if (newPref != null) {
				//get germplasm's existing preferred name/abbreviation, set as alternative name, change nstat to 0
				Name oldPref = null;
				int newNstat = 0; //nstat to be assigned to newPref: 1 for Name, 2 for Abbreviation
		...
		try {
			...
			for (Name name : names) {
				if (operation == Operation.ADD) {
					...
					//Check if Name is a local DB record. Throws exception if Name is a central DB record.
					dao.validateId(name);
				}
	 ...
	 *	l)	L-N becomes L^N when there is only one дад in the name and L is not preceded by a space
	...
	public static String standardaizeName(String name)
	{
		...
		for(int ctr = 0; ctr < stringLength; ctr++)
		{
			...
			else if(Character.isLetter(currentChar))
			{
				if(ctr+1 < stringLength)
				{
					...
					if(Character.isDigit(nextChar))
					{
						...
						//check if there is a special character before the letter
						if(ctr-1 >= 0)
						{
							char previousChar = toreturn.charAt(ctr-1);
			...
			else if(Character.isDigit(currentChar))
			{
				if(ctr+1 < stringLength)
				{
					...
					if(Character.isLetter(nextChar))
					{
						...
						//check if there is a special character before the number
						if(ctr-1 >= 0)
						{
							char previousChar = toreturn.charAt(ctr-1);
							if(GermplasmDataManagerImpl.isGermplasmNameSpecialChar(previousChar))
			...
			else if(currentChar == '-')
			{
				if((ctr-1 >= 0) && (ctr+1 < stringLength))
				{
					// L-N becomes L^N when there is only one дад in the name and L is not preceded by a space
					char nextChar = toreturn.charAt(ctr+1);
					char previousChar = toreturn.charAt(ctr-1);
					
					if(Character.isLetter(previousChar)
						&& Character.isDigit(nextChar)
						//if there is only one '-' in the string then the last occurrence of that char is the only occurrence
	...
	/**
	 * Returns true if the given char is considered a special character based on ICIS
	 * Germplasm Name standardization rules.  Returns false otherwise.
	 * 
	 * @param c
	 * @return
	 */
	...
	/**
	 * Given a GermplasmPedigreeTreeNode and the level of the desired tree, add parents to the node recursively until
	 * the specified level of the tree is reached.
	 * 
	 * @param node
	 * @param level
	...
	public GermplasmPedigreeTree getDerivativeNeighborhood(Integer gid,
			int numberOfStepsBackward, int numberOfStepsForward) throws QueryException
	{
	...
	/**
	 * Recursive function which gets the root of a derivative neighborhood by tracing
	 * back through the source germplasms.  The function stops when the steps
	 * are exhausted or a germplasm created by a generative method is encountered,
		...
		try {
			...
			for(Attribute attribute : attributes) {
				if (operation == Operation.ADD) {
					...
					//Check if Attribute is a local DB record. Throws exception if Attribute is a central DB record.
	...
	public boolean updateProgenitor(Integer gid, Integer progenitorId, Integer progenitorNumber) throws QueryException
	{
		...
		if(progenitorNumber == 1 || progenitorNumber == 2)
		{
			...
			{
				throw new QueryException("The gid supplied as parameter does not refer to a local record. Only local records may be updated.");
			}
		}
	...
	/**
	@Override
	public List<Germplasm> findGermplasmByExample(Germplasm sample, int start,
			int numOfRows)
	{
		GermplasmDAO dao = new GermplasmDAO();
		dao.setSession(hibernateUtil.getCurrentSession());
		return dao.findByExample(sample, start, numOfRows);
	}
	...
	public int countGermplasmByExample(Germplasm sample)
	{
		GermplasmDAO dao = new GermplasmDAO();
		dao.setSession(hibernateUtil.getCurrentSession());
		return dao.countByExample(sample).intValue();
	}
	**/
}
