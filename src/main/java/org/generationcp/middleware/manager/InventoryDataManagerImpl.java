...
public class InventoryDataManagerImpl extends DataManager<Lot> implements
	...
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
	return dao
		.findByEntityTypeAndEntityId(type, entityId, start, numOfRows);
    }
	...
	LotDAO dao = new LotDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
	return dao.findByEntityTypeAndLocationId(type, locationId, start,
		numOfRows);
    }
	...
	LotDAO dao = new LotDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
	return dao.findByEntityTypeAndEntityIdAndLocationId(type, entityId,
		locationId, start, numOfRows);
    }
	...
	LotDAO dao = new LotDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
	return dao.countByEntityTypeAndEntityIdAndLocationId(type, entityId,
		locationId).intValue();
    }
    ...
    private int addOrUpdateLot(List<Lot> lots, Operation operation)
	...
	try {
	    ...
	    throw new QueryException("Error encountered while saving Lot: "
		    + ex.getMessage(), ex);
	} finally {
    ...
    public int addTransaction(
	    org.generationcp.middleware.pojos.Transaction transaction)
	    throws QueryException {
    ...
    private int addOrUpdateTransaction(
	    List<org.generationcp.middleware.pojos.Transaction> transactions,
	...
	try {
	    ...
	    throw new QueryException(
		    "Error encountered while saving Transaction: "
			    + ex.getMessage(), ex);
	} finally {
	    hibernateUtilForLocal.closeCurrentSession();
	}
    ...
    @Override
    public org.generationcp.middleware.pojos.Transaction getTransactionById(
	    Integer id) {
	TransactionDAO dao = new TransactionDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
    ...
    @Override
    public Set<org.generationcp.middleware.pojos.Transaction> findTransactionsByLotId(
	    Integer id) {
	LotDAO dao = new LotDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
    ...
    @Override
    public List<org.generationcp.middleware.pojos.Transaction> getAllReserveTransactions(
    ...
    @Override
    public List<org.generationcp.middleware.pojos.Transaction> getAllDepositTransactions(
	    int start, int numOfRows) {
	TransactionDAO dao = new TransactionDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
    ...
    @Override
    public List<org.generationcp.middleware.pojos.Transaction> getAllReserveTransactionsByRequestor(
    ...
    @Override
    public List<org.generationcp.middleware.pojos.Transaction> getAllDepositTransactionsByDonor(
	    Integer personId, int start, int numOfRows) {
	TransactionDAO dao = new TransactionDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
	...
	GermplasmDataManagerImpl germplasmManager = new GermplasmDataManagerImpl(
		this.hibernateUtilForLocal, this.hibernateUtilForCentral);
	TraitDataManagerImpl traitManager = new TraitDataManagerImpl(
		this.hibernateUtilForLocal, this.hibernateUtilForCentral);

	TransactionDAO dao = new TransactionDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());
	List<org.generationcp.middleware.pojos.Transaction> transactions = dao
		.getAllUncommitted(start, numOfRows);
	...
	for (org.generationcp.middleware.pojos.Transaction t : transactions) {
	    ...
	    Location location = germplasmManager.getLocationByID(t.getLot()
		    .getLocationId());
	...
	List<org.generationcp.middleware.pojos.Transaction> transactions = getAllReserveTransactions(
		start, numOfRows);
	for (org.generationcp.middleware.pojos.Transaction t : transactions) {
	    TransactionReportRow row = new TransactionReportRow();
	    row.setDate(t.getDate());
	...
	List<org.generationcp.middleware.pojos.Transaction> transactions = dao
		.getAllWithdrawals(start, numOfRows);
	for (org.generationcp.middleware.pojos.Transaction t : transactions) {
	    TransactionReportRow row = new TransactionReportRow();
	    row.setDate(t.getDate());
	    row.setQuantity(t.getQuantity());
	...
	for (Object o : results) {
		...
		Location location = germplasmManager
			.getLocationByID((Integer) result[3]);
		row.setLocationOfLot(location);
	...
	for (org.generationcp.middleware.pojos.Transaction t : dao.getEmptyLot(
		start, numOfRows)) {
    ...
    public List<LotReportRow> generateReportOnLotsWithMinimumAmount(
	    long minAmount, int start, int numOfRows) {
	TransactionDAO dao = new TransactionDAO();
	dao.setSession(this.hibernateUtilForLocal.getCurrentSession());

	List<Lot> lotsWithMinimunAmount = new ArrayList<Lot>();
	for (org.generationcp.middleware.pojos.Transaction t : dao
	...
	return generateReportOnLotsByEntityTypeAndEntityId(type, entityIds,
		start, numOfRows);
    }
	...
	for (Integer entityId : entityIds) {
	    List<Lot> lotsForEntityId = findLotsByEntityTypeAndEntityId(type,
		    entityId, start, numOfRows);
	...
	for (Lot lot : listOfLots) {
	    ...
	    Location location = germplasmManager.getLocationByID(lot
		    .getLocationId());
	    row.setLocationOfLot(location);
