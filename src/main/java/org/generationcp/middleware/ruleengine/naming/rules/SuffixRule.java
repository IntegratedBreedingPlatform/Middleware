...
public class SuffixRule extends OrderedRule<NamingRuleExecutionContext> {
	...
	public Object runRule(NamingRuleExecutionContext context) throws RuleException {
		...
		String suffix = deprecatedAdvancingSource.getBreedingMethod().getSuffix();
		...
		for (int i = 0; i < input.size(); i++) {
			input.set(i, processCodeService.applyProcessCode(input.get(i), suffix, deprecatedAdvancingSource).get(0));
		}
