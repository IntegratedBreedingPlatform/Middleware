...
public abstract class VariableDetailsFactory <T extends VariableDetails> {
	...
	public List<T> createDetails(DmsProject project, List<CVTermRelationship> relationships) {
		
		List<T> variableList = new ArrayList<T>();

		if (project != null) {
			List<Variable> variables = new Variables(project.getProperties(), relationships).getVariables();
