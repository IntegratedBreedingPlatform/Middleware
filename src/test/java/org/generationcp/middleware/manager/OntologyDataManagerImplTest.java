...
public class OntologyDataManagerImplTest extends DataManagerIntegrationTest {
	...
	public void testGetStandardVariableSummaryLocal() throws MiddlewareQueryException {
		...
		myOwnPlantHeight
				.setMethod(new Term(16010, "Soil to tip at maturity", "Soil to tip at maturity"));
		...
		myOwnPlantHeight.setStoredIn(
				new Term(1043, "Observation variate", "Phenotypic data stored in phenotype.value"));
	...
	private void assertVariableDataMatches(StandardVariable details,
			StandardVariableSummary summary) {
		assertEquals(new Integer(details.getId()), summary.getId());
		assertEquals(details.getName(), summary.getName());
		assertEquals(details.getDescription(), summary.getDescription());
		...
		assertEquals(details.getPhenotypicType(), summary.getPhenotypicType());
	}

	private void assertTermDataMatches(Term termDetails, TermSummary termSummary) {
		assertEquals(new Integer(termDetails.getId()), termSummary.getId());
		assertEquals(termDetails.getName(), termSummary.getName());
		assertEquals(termDetails.getDefinition(), termSummary.getDefinition());
	}
	...
	public void testStandardVariableCache() throws Exception {
		...
		manager.getStandardVariable(
				STD_VARIABLE_ID);        // First call to getStandardVariable() will put the value to the cache
		manager.getStandardVariable(
				STD_VARIABLE_ID);        // Second (and subsequent) calls will retrieve the value from the cache
	}
	...
	public void testUpdateStandardVariableEnumeration() throws Exception {
		...
		standardVariable = manager.getStandardVariable(standardVariableId);
		Debug.printObject(INDENT, standardVariable);
		assertTrue(standardVariable.getEnumeration(validValue.getId()).getDescription()
				.equals(description));
	...
	public void testFindStandardVariablesByNameOrSynonym() throws Exception {
		...
		Set<StandardVariable> standardVariables = manager
				.findStandardVariablesByNameOrSynonym("foo bar");
	...
	public void testFindStandardVariablesByNameOrSynonymWithProperties() throws Exception {
		...
		Set<StandardVariable> standardVariables = manager
				.findStandardVariablesByNameOrSynonym("Accession name");
		assertSame(standardVariables.size(), 1);
		for (StandardVariable stdVar : standardVariables) {
			stdVar.print(INDENT);
	...
	public void testGetMethodsForTrait() throws Exception {
		...
		//2nd test
		stdVar = manager.findStandardVariableByTraitScaleMethodNames("Germplasm entry", "Number",
				"Enumerated");
	...
	public void testGetStandardVariablesForPhenotypicType() throws Exception {
		...
		for (Object key : standardVariables.keySet()) {
			Debug.println(
					key + " : " + standardVariables.get(key).getId() + " : " + standardVariables
							.get(key).toString());
		}
	...
	/**
	 * This tests an expected property of the ontology data manager to return a non empty map,
	 * even for entries that cannot be matched to a standard variable in the database
	 *
	 * @throws Exception
	 */
	@Test
	public void testGetStandardVariablesInProjectsNonNullEvenIfNotPresent() throws Exception {
		...
		assertNotNull(
				...
				results);
		assertTrue("Application is unable to return non empty output even on non present variables",
		...
		for (Map.Entry<String, List<StandardVariable>> entry : results.entrySet()) {
			...
			assertNotNull(
					...
					vars);
			assertTrue("Application shouldn't be able to give values for dummy input",
					vars.size() == 0);
		}
	}

	/**
	 * This tests the ability of the application to retrieve the standard variables associated with known central headers
	 *
	 * @throws Exception
	 */
	@Test
	public void testGetStandardVariablesInProjectsKnownCentralHeaders() throws Exception {
		List<String> headers = Arrays.asList(CENTRAL_COMMON_HEADERS);
	...
	public void testGetStandardVariablesForNewlyCreatedEntries() throws Exception {
		...
		StandardVariable dummyVariable = constructDummyStandardVariable();
		...
		try {
			...
			// make test clean by deleting the dummy variable created during start of test
	...
	protected StandardVariable constructDummyStandardVariable() throws Exception {
		...
		Term propertyTerm = manager.findTermByName("Yield", CvId.PROPERTIES);
		...
		Term methodTerm = manager.findTermByName("Counting", CvId.METHODS);
		standardVariable.setMethod(methodTerm);

		Term storedInTerm = manager.getTermById(TermId.OBSERVATION_VARIATE.getId());
		standardVariable.setStoredIn(storedInTerm);

		Term dataType = manager.getTermById(TermId.NUMERIC_VARIABLE.getId());
	...
	public void testAddProperty() throws Exception {
		...
		Debug.println(INDENT,
				"testAddProperty(name=" + name + ", definition=" + definition + ", isA=" + isA
						+ "): ");
	...
	public void testGetStandardVariablesByTraitClass() throws MiddlewareQueryException {
		List<StandardVariable> vars = manager
				.getStandardVariables(Integer.valueOf(1410), null, null, null);
	...
	public void testGetStandardVariablesByProperty() throws MiddlewareQueryException {
		List<StandardVariable> vars = manager
				.getStandardVariables(null, Integer.valueOf(20109), null, null);
	...
	public void testGetStandardVariablesByMethod() throws MiddlewareQueryException {
		List<StandardVariable> vars = manager
				.getStandardVariables(null, null, Integer.valueOf(20643), null);
	...
	public void testGetStandardVariablesByScale() throws MiddlewareQueryException {
		List<StandardVariable> vars = manager
				.getStandardVariables(null, null, null, Integer.valueOf(20392));
	...
	public void testAddOrUpdateTermAndRelationshipFoundInCentral() throws Exception {
		String name = "Season";
		String definition = "Growing Season " + (int) (Math.random()
				* 100); // add random number to see the update
		try {
			manager.addOrUpdateTermAndRelationship(name, definition, CvId.PROPERTIES,
					TermId.IS_A.getId(), 1340, null);
		} catch (MiddlewareException e) {
			Debug.println(INDENT, "MiddlewareException expected: \"" + e.getMessage() + "\"");
			assertTrue(e.getMessage()
					.contains(" is retrieved from the central database and cannot be updated"));
		}
	}
	...
	public void testAddOrUpdateTermAndRelationshipNotInCentral() throws Exception {
		...
		String definition = "Study condition NEW class " + (int) (Math.random()
				* 100); // add random number to see the update
		Term origTerm = manager.findTermByName(name, CvId.PROPERTIES);
		Term newTerm = manager.addOrUpdateTermAndRelationship(name, definition, CvId.PROPERTIES,
				TermId.IS_A.getId(), 1340, null);
	...
	public void testUpdateTermAndRelationshipFoundInCentral() throws Exception {
		String name = "Slope";
		String definition =
				"Land slope " + (int) (Math.random() * 100); // add random number to see the update
		Term origTerm = manager.findTermByName(name, CvId.PROPERTIES);
		try {
			manager.updateTermAndRelationship(new Term(origTerm.getId(), name, definition),
					TermId.IS_A.getId(), 1340);
		} catch (MiddlewareException e) {
			Debug.println(INDENT, "MiddlewareException expected: \"" + e.getMessage() + "\"");
	...
	public void testUpdateTermAndRelationshipNotInCentral() throws Exception {
		...
		String definition = "Slope NEW class " + (int) (Math.random()
				* 100); // add random number to see the update

		Term origTerm = manager.findTermByName(name, CvId.PROPERTIES);
		if (origTerm == null) { // first run, add before update
			origTerm = manager.addOrUpdateTermAndRelationship(name, definition, CvId.PROPERTIES,
					TermId.IS_A.getId(), 1340, null);
		}
	...
	public void testAddOrUpdateTermFoundInCentral() throws Exception {
		String name = "Score";
		String definition =
				"Score NEW " + (int) (Math.random() * 100); // add random number to see the update
		try {
	...
	public void testAddOrUpdateTermNotInCentral() throws Exception {
		String name = "Real";
		String definition = "Real Description NEW " + (int) (Math.random()
				* 100); // add random number to see the update
	...
	public void testUpdateTermNotInCentral() throws Exception {
		...
		String definition =
				"Integer NEW " + (int) (Math.random() * 100); // add random number to see the update
	...
	public void testGetStandardVariableIdByTermId() throws Exception {
		...
		Integer stdVariableId = manager
				.getStandardVariableIdByTermId(stdVariable.getProperty().getId(),
						TermId.HAS_PROPERTY);
	...
	public void testDeleteTermAndRelationship() throws Exception {
		...
		term = manager.addTraitClass(name, definition, TermId.ONTOLOGY_TRAIT_CLASS.getId())
				.getTerm();
		manager.deleteTermAndRelationship(term.getId(), CvId.IBDB_TERMS, TermId.IS_A.getId(),
	...
	public void testDeleteOntology() throws Exception {
        ...
        */
		...
		Term termParent = manager
				.addTraitClass(name, definition, TermId.ONTOLOGY_TRAIT_CLASS.getId()).getTerm();
		...
		try {
			manager.deleteTermAndRelationship(termParent.getId(), CvId.IBDB_TERMS,
		...
		manager.deleteTermAndRelationship(termChild.getId(), CvId.IBDB_TERMS, TermId.IS_A.getId(),
				TermId.ONTOLOGY_TRAIT_CLASS.getId());
		manager.deleteTermAndRelationship(termParent.getId(), CvId.IBDB_TERMS, TermId.IS_A.getId(),
				TermId.ONTOLOGY_TRAIT_CLASS.getId());
