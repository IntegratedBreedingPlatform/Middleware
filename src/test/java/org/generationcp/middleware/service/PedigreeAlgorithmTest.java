...
/**
 * Test essentially compares our pedigree generation results of the new algorithm vs the old one.
 * Please note this test will not work if we change the existing algorithm.
 *
 */
public class PedigreeAlgorithmTest extends IntegrationTestBase {
	...
	public void compareGermplasmGeneratedByDifferentAlgorithms(final String methodName) {
		final List<Germplasm> randomlySelectGermplasm = randomlySelectGermplasm(10, methodName);
		for (Germplasm germplasm : randomlySelectGermplasm) {
			final String pedigreeStringGeneratedByOriginalAlgorithm = oldPedigreeService.getCrossExpansion(germplasm.getGid(), newCrossExpansionProperties);
			final String pedigreeStringGeneratedByNewAlgorithm = pedigreeService.getCrossExpansion(germplasm.getGid(), newCrossExpansionProperties);
			Assert.assertEquals("Asssertion for GID " + germplasm.getGid() + " failed.", pedigreeStringGeneratedByOriginalAlgorithm, pedigreeStringGeneratedByNewAlgorithm);
		}
	}
	...
	private List<Germplasm> randomlySelectGermplasm(final int numberOfRows, final String methodName) {
		...
		System.out.println(String.format("Getting '%d' number of rows from '%d' starting at '%d' avaiable for method name like '%s'", numberOfRows,
				numberOfGermplasmWithMethodName, germplasmStartIndex, methodName));
