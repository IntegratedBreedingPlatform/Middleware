...
public class FieldbookServiceTest {
    ...
    public void testGetCompleteDataset() throws MiddlewareQueryException{
    	...
    	studyDetails.setStudyName("Test Name");
    	workbook.setStudyDetails(studyDetails);
    	Mockito.when(builder.buildCompleteDataset(Mockito.anyInt(), Mockito.anyBoolean())).thenReturn(workbook);
    	impl.getCompleteDataset(1, true);
    	Mockito.verify(impl, Mockito.times(1)).setOrderVariableByRank(Mockito.any(Workbook.class), Mockito.anyInt());    	
