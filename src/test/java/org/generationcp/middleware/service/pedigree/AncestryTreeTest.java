...
public class AncestryTreeTest {
		...
		this.methodCache =
				new FunctionBasedGuavaCacheLoader<CropMethodKey, Method>(CacheBuilder.newBuilder().maximumSize(100000)
					...
					public Method apply(CropMethodKey key) {
						return AncestryTreeTest.this.methodsMap.get(key);
					}
				});
	}
		...
		this.germplasmCache =
				new FunctionBasedGuavaCacheLoader<CropGermplasmKey, Germplasm>(CacheBuilder.newBuilder().maximumSize(100000)
					...
					public Germplasm apply(CropGermplasmKey key) {
						return AncestryTreeTest.this.germplasmMap.get(key);
					}
				});
	}
	...
	public void testPedigreeTreeGeneration() throws Exception {
		...
		final AncestryTree pedigreeTree = new AncestryTree(this.germplasmCache, this.methodCache, AncestryTreeTest.MAIZE);
	...
	private void compareGeneratedNodes(final Germplasm generateRandomGermplasm, final GermplasmNode resultNode) {
		...
		final CropGermplasmKey femaleGermplasmKey = new CropGermplasmKey(AncestryTreeTest.MAIZE, generateRandomGermplasm.getGpid1());
		this.compareGeneratedNodes(this.germplasmMap.get(femaleGermplasmKey), resultNode.getFemaleParent());

		final CropGermplasmKey maleGermplasmKey = new CropGermplasmKey(AncestryTreeTest.MAIZE, generateRandomGermplasm.getGpid2());
	...
	private Germplasm generateTestGermplasm(final int gid, final int methodId) {
		...
		this.germplasmMap.put(new CropGermplasmKey(AncestryTreeTest.MAIZE, gid), germplasm);
		this.methodsMap.put(new CropMethodKey(AncestryTreeTest.MAIZE, methodId), method);
		return germplasm;
	}
