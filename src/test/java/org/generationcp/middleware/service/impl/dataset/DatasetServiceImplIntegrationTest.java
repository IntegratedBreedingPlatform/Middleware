...
public class DatasetServiceImplIntegrationTest extends IntegrationTestBase {
	...
	public void testGetInstanceObservationUnitRowsMap() {
		final Map<Integer, List<ObservationUnitRow>> instanceObsUnitRowMap =
	...
	public void testGetObservationUnitRowsAsMapList() {
		...
		final List<Map<String, Object>> rowsAsListMap =
			this.datasetService.getObservationUnitRowsAsMapList(this.studyId, this.subObsDatasetId,
				searchDto, new PageRequest(0, Integer.MAX_VALUE));
	...
	public void testGetTrialNumberPlotNumberObservationUnitIdTable() {
		...
		Table<Integer, Integer, Integer> observationUnitIdsPlotNumberTable =
			this.datasetService.getTrialNumberPlotNumberObservationUnitIdTable(this.plotDatasetId, trialInstances, plotNumbers);
		...
		observationUnitIdsPlotNumberTable =
			this.datasetService.getTrialNumberPlotNumberObservationUnitIdTable(this.plotDatasetId, trialInstances, nonExistentPlotNumbers);
		...
		observationUnitIdsPlotNumberTable = this.datasetService.getTrialNumberPlotNumberObservationUnitIdTable(this.plotDatasetId,
			Collections.singleton(nonExistentTrialInstance), plotNumbers);
		Assert.assertTrue(observationUnitIdsPlotNumberTable.isEmpty());
	}
	...
	public void testGetObservationSetColumnsForObservations() {
		...
		final Map<Integer, MeasurementVariable> columnsMap =
			columns.stream().collect(Collectors.toMap(MeasurementVariable::getTermId, Function
				.identity()));
	...
	public void testGetObservationSetColumnsSummaryStatistics() {
		...
		final List<MeasurementVariable> columns =
			this.datasetService.getObservationSetColumns(this.studyId, summaryStatisticsDatasetId, false);
		final Map<Integer, MeasurementVariable> columnsMap =
			columns.stream().collect(Collectors.toMap(MeasurementVariable::getTermId, Function
				.identity()));
	...
	private void createTestStudyWithSubObservations() {
		...
		this.studyId = this.dataSetupTest.createNurseryForGermplasm(this.commonTestProject.getUniqueID(), gids, "ABCD",
			DataSetupTest.NUMBER_OF_GERMPLASM, 2);
	...
	private VariableFilter createVariableFilter(final String variableType, final List<String> analysisMethodNames) {
		final Variable testVariable =
			this.createTestVariable(RandomStringUtils.randomAlphanumeric(10), RandomStringUtils.randomAlphanumeric(10),
