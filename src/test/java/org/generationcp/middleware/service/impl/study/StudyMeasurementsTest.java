...
public class StudyMeasurementsTest {
	...
	private static final String ENTRY_TYPE = String.valueOf(SystemDefinedEntryType.CHECK_ENTRY.getEntryTypeCategoricalId());
	private static final List<String> STRING_COLUMNS =
			Lists.newArrayList("nd_experiment_id", "TRIAL_INSTANCE", "ENTRY_TYPE", "GID", "DESIGNATION", "ENTRY_NO", "ENTRY_CODE", "REP_NO",
					"PLOT_NO", "BLOCK_NO", "ROW", "COL", "FIELDMAP COLUMN", "FIELDMAP RANGE", "SUM_OF_SAMPLES");
	private final List<String> germplasmDescriptors = Lists.newArrayList(StudyMeasurementsTest.STOCK_ID);
	private final List<String> designFactors = Lists.newArrayList(StudyMeasurementsTest.FACT1, StudyMeasurementsTest.FACT2);
	private final List<MeasurementVariableDto> testTraits = Arrays.asList(new MeasurementVariableDto(1, StudyMeasurementsTest.TRAIT1),
	...
	public void setup() {
		...
		this.testRow1 = new Object[] {StudyMeasurementsTest.PLOT_IDENTIFIER1, StudyMeasurementsTest.TRIAL_INSTANCE,
				...
				StudyMeasurementsTest.ROW1_TRAIT1_PHENOTYPE_ID, StudyMeasurementsTest.ROW1_TRAIT2_VALUE,
				StudyMeasurementsTest.ROW1_TRAIT2_PHENOTYPE_ID, StudyMeasurementsTest.STOCKID1, StudyMeasurementsTest.FACT1_VALUE1,
		...
		this.testRow2 = new Object[] {StudyMeasurementsTest.PLOT_IDENTIFIER2, StudyMeasurementsTest.TRIAL_INSTANCE,
				...
				StudyMeasurementsTest.ROW2_TRAIT1_PHENOTYPE_ID, StudyMeasurementsTest.ROW2_TRAIT2_VALUE,
				StudyMeasurementsTest.ROW2_TRAIT2_PHENOTYPE_ID, StudyMeasurementsTest.STOCKID2, StudyMeasurementsTest.FACT1_VALUE2,
	...
	public void allPlotsMeasurementQueryRetrievesDataCorrectly() throws Exception {
		...
		final List<ObservationDto> returnedMeasurements =
				this.studyMeasurements.getAllMeasurements(StudyMeasurementsTest.PROJECT_IDENTIFIER, this.testTraits,
						this.germplasmDescriptors, this.designFactors, instanceId, pageNumber, pageSize, null, null);
		...
		Assert.assertEquals("Make sure that we have all measurements returned", this.allMeasurements.size(), returnedMeasurements.size());
		...
		this.verifyObservationValues(measurement1, StudyMeasurementsTest.PLOT_IDENTIFIER1, StudyMeasurementsTest.GID1,
				...
				Arrays.asList(new ImmutablePair<String, String>(StudyMeasurementsTest.STOCK_ID, StudyMeasurementsTest.STOCKID1)),
				Arrays.asList(new ImmutablePair<String, String>(StudyMeasurementsTest.FACT1, StudyMeasurementsTest.FACT1_VALUE1),
						new ImmutablePair<String, String>(StudyMeasurementsTest.FACT2, StudyMeasurementsTest.FACT2_VALUE1)));
	...
	public void singlePlotMeasurementsQueryRetrievesDataCorrectly() throws Exception {
		Mockito.when(this.session.createSQLQuery(
				new ObservationQuery().getSingleObservationQuery(this.testTraits, this.germplasmDescriptors, this.designFactors)))
				.thenReturn(this.mockSqlQueryForSingleMeasurement);

		final List<ObservationDto> returnedMeasurements = this.studyMeasurements.getMeasurement(StudyMeasurementsTest.PROJECT_IDENTIFIER,
				this.testTraits, this.germplasmDescriptors, this.designFactors, StudyMeasurementsTest.PLOT_IDENTIFIER1);
	...
	private void verifyObservationValues(final ObservationDto measurement, final int plotIdentifier, final int gid, final String entryNo,
			final String seedSource, final String plotNo, final String plotId, final List<ImmutablePair<Integer, String>> traits,
			final List<ImmutablePair<String, String>> germplasmDescriptors, final List<ImmutablePair<String, String>> designFactors) {
		Assert.assertEquals("Make sure the Experiment ID is correct", new Integer(plotIdentifier), measurement.getMeasurementId());
		...
		Assert.assertEquals("Make sure the Entry Type is correct", StudyMeasurementsTest.ENTRY_TYPE, measurement.getEntryType());
		...
		Assert.assertEquals("Make sure the Rep # is correct", StudyMeasurementsTest.REP_NO, measurement.getRepitionNumber());
		Assert.assertEquals("Make sure the Block # is correct", StudyMeasurementsTest.BLOCK_NO, measurement.getBlockNumber());
		...
		Assert.assertEquals("Make sure the Fielmap Range is correct", StudyMeasurementsTest.FIELDMAP_RANGE, measurement.getFieldMapRange());
		Assert.assertEquals("Make sure the Sum of Samples is correct", StudyMeasurementsTest.SUM_OF_SAMPLES, measurement.getSamples());
		...
		for (final Pair<Integer, String> trait : traits) {
			...
			Assert.assertEquals("Make sure the Trait Phenotype Id is correct", trait.getLeft(), traitMeasurement.getPhenotypeId());
			Assert.assertEquals("Make sure the Trait Value is correct", trait.getRight(), traitMeasurement.getVariableValue());
		}
		final Iterator<Pair<String, String>> germplasmDescripIterator = measurement.getAdditionalGermplasmDescriptors().iterator();
		...
		for (final Pair<String, String> trait : designFactors) {
			...
			Assert.assertEquals("Make sure the Design Factor Value is correct", trait.getRight(), designFactor.getRight());
		}
	}
