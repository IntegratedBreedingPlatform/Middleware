...
public class ObservationsTest {
	...
	public void insertTrait() throws Exception {
		...
		Mockito.when(this.measurementDto.getTriatValue()).thenReturn("Test Value");
		Mockito.when(this.measurementDto.getTrait()).thenReturn(new TraitDto(999, "Test Trait"));
		Mockito.when(this.mockSession.save(Matchers.isA(Phenotype.class))).thenAnswer(new Answer<Serializable>() {
		...
		Mockito.verify(this.mockSession, Mockito.times(1)).save(Matchers.isA(Phenotype.class));
		Mockito.verify(this.mockSession, Mockito.times(1)).save(Matchers.isA(ExperimentPhenotype.class));

		Mockito.verify(this.mockSession, Mockito.times(1)).update(Matchers.isA(Phenotype.class));
	...
	public void updateTrait() throws Exception {
		...
		Mockito.when(this.measurementDto.getTriatValue()).thenReturn("Test Value");
		Mockito.when(this.measurementDto.getPhenotypeId()).thenReturn(1);
		Mockito.when(this.measurementDto.getTrait()).thenReturn(new TraitDto(999, "Test Trait"));
		Mockito.when(this.mockSession.get(Phenotype.class, 1)).thenReturn(Mockito.mock(Phenotype.class));
		this.observation.updataObsevationTraits(this.observationDto);
		Mockito.verify(this.mockSession, Mockito.times(1)).update(Matchers.isA(Phenotype.class));
	}
}
