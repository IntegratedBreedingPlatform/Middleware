...
public class ExperimentDesignServiceImplTest extends IntegrationTestBase {
	...
	public void testSaveExperimentDesign_IterativeAndRegeneratePreviousInstance() {
		...
		final Integer geolocationId1 = this.daoFactory.getInstanceDao()
			.getInstancesForInstanceNumbers(studyId, Collections.singletonList(1)).get(0).getNdExperimentId();
		Assert.assertFalse(this.daoFactory.getInstancePropertyDao()
			.getInstanceVariablesMap(this.environmentDatasetId, geolocationId1)
			.containsKey(TermId.BLOCK_ID.getId()));
		this.daoFactory.getInstancePropertyDao().save(this.createEnvironmentProperty(geolocationId1, TermId.BLOCK_ID.getId(), RandomStringUtils.randomAlphabetic(5)));
		Assert.assertTrue(this.daoFactory.getInstancePropertyDao()
			.getInstanceVariablesMap(this.environmentDatasetId, geolocationId1)
			.containsKey(TermId.BLOCK_ID.getId()));
		...
		this.verifyPlotVariablesWereSaved();
		this.verifyGeolocationPropRecords(true, Arrays.asList(1, 2, 3));
		final Map<Integer, String> map = this.daoFactory.getInstancePropertyDao()
			.getInstanceVariablesMap(this.environmentDatasetId, geolocationId1);
	...
	public void testDeleteExperimentDesign() {
		...
		final Integer environmentId = this.daoFactory.getInstanceDao()
			.getInstancesForInstanceNumbers(studyId, Collections.singletonList(1)).get(0).getNdExperimentId();
		this.daoFactory.getInstancePropertyDao().save(this.createEnvironmentProperty(environmentId, TermId.BLOCK_ID.getId(), RandomStringUtils.randomAlphabetic(5)));
		...
		Assert.assertTrue(this.daoFactory.getInstancePropertyDao()
			.getInstanceVariablesMap(this.environmentDatasetId, environmentId)
			.containsKey(TermId.BLOCK_ID.getId()));
	...
	private void verifyGeolocationPropRecords(final boolean shouldExist, final List<Integer> instanceNumbers) {
		final List<ExperimentModel> environments = this.daoFactory.getInstanceDao().getInstances(studyId);
