...
public class DataSetupTest extends IntegrationTestBase {
	...
	private void createNursery(final String programUUID, final String cropPrefix) {
		...
		this.createNurseryForGermplasm(programUUID, gids, cropPrefix, DataSetupTest.NUMBER_OF_GERMPLASM, 2);
	}

	public int createNurseryForGermplasm(final String programUUID, final Integer[] gids, final String cropPrefix,
		final int numberOfGermplasm, final int replicationNumber) {
		...
		conditions.add(
			this.createMeasurementVariable(TermId.TRIAL_INSTANCE_FACTOR.getId(), "TRIAL_INSTANCE", "Trial instance - enumerated (number)",
		...
		conditions.add(
			this.createMeasurementVariable(TermId.LATITUDE.getId(), "SITE_LAT", "Latitude of site - observed (decimal + for N, - for S)",
		...
		conditions.add(this.createMeasurementVariable(TermId.LONGITUDE.getId(), "SITE_LONG",
			"Longitude of site - observed (decimal + for E , - for W)",
		...
		conditions.add(
			this.createMeasurementVariable(TermId.GEODETIC_DATUM.getId(), "SITE_DATUM", "Geodetic datum for lat, long - observed (text)",
		...
		conditions.add(this.createMeasurementVariable(TermId.LOCATION_ID.getId(), TermId.LOCATION_ID.name(),
			"Location - selected (DBID)", "Location", DataSetupTest.ASSIGNED, "Location id",
			"Location", null, null, PhenotypicType.TRIAL_ENVIRONMENT, true));
		...
		for (int repNo = 1; repNo <= replicationNumber; repNo++) {
			for (int i = 0; i < numberOfGermplasm; i++) {
				...
				final MeasurementData entryTypeData =
					new MeasurementData(entryTypeFactor.getLabel(), SystemDefinedEntryType.TEST_ENTRY.getEntryTypeValue());
