...
public class PhenotypeDaoTest {
	...
	public void testGetNumericTraitInfoList() {
		...
		final String expectedSql = "SELECT p.observable_id, COUNT(DISTINCT env.nd_experiment_id) AS location_count,"
			...
			+ "  LEFT JOIN nd_experiment plot ON plot.nd_experiment_id = e.parent_id and plot.type_id = 1155"
			+ " INNER JOIN project pr ON pr.project_id = e.project_id"
			+ "  INNER JOIN project env_ds ON env_ds.study_id = pr.study_id"
			+ " AND env_ds.dataset_type_id = " + DatasetTypeEnum.SUMMARY_DATA.getId() + ""
	...
	public void testGetTraitInfoCountsForTraits() {
		...
		final String expectedSql = "SELECT p.observable_id,"
			+ " COUNT(DISTINCT env.nd_experiment_id) AS location_count,"
			+ " COUNT(DISTINCT s.dbxref_id) AS germplasm_count,"
			+ " COUNT(DISTINCT e.nd_experiment_id) AS observation_count  FROM phenotype p"
			+ "  INNER JOIN nd_experiment e ON e.nd_experiment_id = p.nd_experiment_id"
			+ "  LEFT JOIN nd_experiment plot ON plot.nd_experiment_id = e.parent_id and plot.type_id = 1155"
			+ " INNER JOIN project pr ON pr.project_id = e.project_id  "
			+ "INNER JOIN project env_ds ON env_ds.study_id = pr.study_id AND env_ds.dataset_type_id = "
	...
	private String getObservationsForTraitMainQuery() {
		return "SELECT p.observable_id, s.dbxref_id, env.nd_experiment_id, p.value  FROM phenotype p"
			+ "  INNER JOIN nd_experiment e ON e.nd_experiment_id = p.nd_experiment_id"
			+ "  LEFT JOIN nd_experiment plot ON plot.nd_experiment_id = e.parent_id and plot.type_id = 1155"
			+ " INNER JOIN project pr ON pr.project_id = e.project_id  INNER JOIN project env_ds ON env_ds.study_id = pr.study_id"
			+ " AND env_ds.dataset_type_id = " + DatasetTypeEnum.SUMMARY_DATA.getId() + " "
			+ " INNER JOIN nd_experiment env ON env_ds.project_id = env.project_id AND env.type_id = 1020"
			+ "    AND (e.parent_id = env.nd_experiment_id OR plot.parent_id = env.nd_experiment_id)"
			+ "  INNER JOIN stock s ON e.stock_id = s.stock_id"
