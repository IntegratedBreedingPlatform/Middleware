...
public class GermplasmDAOTest extends IntegrationTestBase {
	...
	public void testCountMatchGermplasmInListOnlyOneGidExists() {
		...
		final Germplasm germplasm = this.daoFactory.getGermplasmDao().getById(dummyGid);
	...
	public void testCountMatchGermplasmInListGidListIsNullOrEmpty() {
		...
		final long result1 = this.daoFactory.getGermplasmDao().countMatchGermplasmInList(null);
		Assert.assertEquals("The count should be zero because the gid list is null", 0, (int) result1);

		final long result2 = this.daoFactory.getGermplasmDao().countMatchGermplasmInList(new HashSet<>());
	...
	public void testGetGermplasmDescendantByGIDs() {
		...
		Assert.assertTrue(this.daoFactory.getGermplasmDao().getGermplasmOffspringByGIDs(Collections.singletonList(mParentGID)).size() > 0);
		Assert.assertTrue(this.daoFactory.getGermplasmDao().getGermplasmOffspringByGIDs(Collections.singletonList(fParentGID)).size() > 0);
		Assert.assertFalse(this.daoFactory.getGermplasmDao().getGermplasmOffspringByGIDs(Collections.singletonList(gid)).size() > 0);
	}
	...
	private Attribute saveAttribute(final Germplasm germplasm, final String attributeType) {
		...
		if (cvTerm == null) {
			cvTerm = new CVTerm(null, CvId.VARIABLES.getId(), attributeType, attributeType, null, 0, 0);
