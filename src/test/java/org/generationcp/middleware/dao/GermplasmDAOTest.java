...
public class GermplasmDAOTest extends IntegrationTestBase {
	...
	public void testGetExistingCrossesWithSingleMaleParent() {
		...
		final Germplasm existingCross = GermplasmTestDataInitializer.createGermplasm(20150101, femaleParent.getGid(), maleParent.getGid(), 2, 0, 0, 1, 1, 0, 1, 1, "MethodName", "LocationName");
		this.dao.save(existingCross);
		List<Germplasm> existingCrosses = this.dao.getExistingCrosses(femaleParent.getGid().toString(), 1, Collections.singletonList(maleParent.getGid()), null);
		Assert.assertEquals(existingCross.getGid(), existingCrosses.get(0).getGid());
		//Check if self is excluded
		existingCrosses = this.dao.getExistingCrosses(femaleParent.getGid().toString(), 1, Collections.singletonList(maleParent.getGid()), existingCross.getGid().toString());
	...
	public void testGetExistingCrossesWithMultipleMaleParents() {
		...
		this.dao.save(maleParent2);
		this.progenitorDao.save(new Progenitor(existingCross, 3, maleParent2.getGid()));
		List<Germplasm> existingCrosses = this.dao.getExistingCrosses(femaleParent.getGid().toString(), 1, Arrays.asList(maleParent1.getGid(), maleParent2.getGid()), null);
		Assert.assertEquals(existingCross.getGid(), existingCrosses.get(0).getGid());
		//Check if self is excluded
		existingCrosses = this.dao.getExistingCrosses(femaleParent.getGid().toString(), 1, Arrays.asList(maleParent1.getGid(), maleParent2.getGid()), existingCross.getGid().toString());
		Assert.assertTrue(existingCrosses.isEmpty());
	}
