...
public class SampleGenotypeServiceImplTest extends IntegrationTestBase {
	...
	public void testImportSampleGenotypes() {
		...
		Assert.assertEquals(1, this.sampleGenotypeService.countFilteredSampleGenotypes(new SampleGenotypeSearchRequestDTO(this.study.getProjectId())));
	...
	public void testSearchSampleGenotypes_WithFilters() {
		...
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.GID.getId()), germplasm.getGid().toString());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.DESIG.getId()), germplasm.getPreferredName().getNval());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.TRIAL_INSTANCE_FACTOR.getId()), this.trialInstance1.getDescription());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.ENTRY_NO.getId()), firstInstanceExperimentModels.get(0).getStock().getUniqueName());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.OBS_UNIT_ID.getId()), firstInstanceExperimentModels.get(0).getObsUnitId());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.TAKEN_BY.getId()), this.user.getPerson().getFullName());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.SAMPLE_NAME.getId()), firstInstanceSample.getSampleName());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.SAMPLE_UUID.getId()), firstInstanceSample.getSampleBusinessKey());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredTextValues().put(String.valueOf(TermId.SAMPLING_DATE.getId()), Util.formatDateAsStringValue(firstInstanceSample.getSamplingDate(), Util.FRONTEND_DATE_FORMAT));
		sampleGenotypeSearchRequestDTO.getFilter().setFilteredValues(new HashMap<>());
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredValues().put(String.valueOf(TermId.REP_NO.getId()), Collections.singletonList("1"));
		sampleGenotypeSearchRequestDTO.getFilter().getFilteredValues().put(String.valueOf(TermId.PLOT_NO.getId()), Collections.singletonList("1"));
		...
		Assert.assertEquals("G", sampleGenotypeDTO.getGenotypeDataMap().get(DEFAULT_MARKER_2).getValue());
		Assert.assertEquals("T", sampleGenotypeDTO.getGenotypeDataMap().get(DEFAULT_MARKER_3).getValue());
		Assert.assertEquals(germplasm.getGid().toString(), sampleGenotypeDTO.getGenotypeDataMap().get(variableNameMap.get(TermId.GID.getId())).getValue());
		Assert.assertEquals(germplasm.getPreferredName().getNval(), sampleGenotypeDTO.getGenotypeDataMap().get(variableNameMap.get(TermId.DESIG.getId())).getValue());
		Assert.assertEquals(this.trialInstance1.getDescription(), sampleGenotypeDTO.getGenotypeDataMap().get(variableNameMap.get(TermId.TRIAL_INSTANCE_FACTOR.getId())).getValue());
		Assert.assertEquals(firstInstanceExperimentModels.get(0).getStock().getUniqueName(), sampleGenotypeDTO.getGenotypeDataMap().get(variableNameMap.get(TermId.ENTRY_NO.getId())).getValue());
		Assert.assertEquals(firstInstanceExperimentModels.get(0).getObsUnitId(), sampleGenotypeDTO.getGenotypeDataMap().get(variableNameMap.get(TermId.OBS_UNIT_ID.getId())).getValue());
		...
		Assert.assertEquals(this.user.getPerson().getFirstName() + " " + this.user.getPerson().getLastName(), sampleGenotypeDTO.getTakenBy());
